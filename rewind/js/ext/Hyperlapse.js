  ...
  _renderer = isWebGL() ? new THREE.WebGLRenderer() : new THREE.CanvasRenderer();
	...
	var handleDirectionsRoute = function(response) {
		if(!_is_playing) {
			...
			var segment_length = total_distance/_max_points; // _max_points determines number of points between start and end
			_d = (segment_length < _distance_between_points) ? _d = _distance_between_points : _d = segment_length; // get the larger distance between 2 points
			...
			for(i=0; i<path.length; i++) {
				if(i+1 < path.length) {
					...
					if(r === 0) {
						...
						if(segs > 0) {
							for(var j=0; j<segs; j++) { //j is segment number
								var t = j/segs; // portion of the total path that we proceeded so far. t will be multiplied by distance between a and b.
							...
							r = d-(_d*segs); // r is the last segment that was smaller than the _d (distance bw 2 points)
						} else {
							r = _d*( 1-(d/_d) ); // this is wrong?
						}
					}
